(function () {
  ...
  function montarTexto(row, isPalestra, juizSelecionado, overrides) {
    ...
    const textoCivilPersonalizado = context.isEventoCivil && isPalestra && textos ? String(textos.palestra || "").trim() : "";
    ...
    if (isPalestra) {
      ...
      if (context.isEventoCivil) {
        if (textoCivilPersonalizado) return templateToTokens(textoCivilPersonalizado, context);
